...
updateTodo(elem.id,todo)
  ...
  return (
    <div>
      <button onClick={handleOpen}><AiFillEdit size={25}/></button>
      <Modal
        open={open}
        onClose={handleClose}
...
<div className='flex w-full py-2'>         
              <label htmlFor="priority">Set Priority:</label>
               <select required id="priority" value={todo.priority} onChange={(e)=>handleChange(e)} className='w-[50%] rounded-lg text-center mx-2'>
...
<div className='flex w-full   py-2'>         
            <label htmlFor="Story points">Story Points : </label> <input type="number" value={todo.days}required min={0} className="w-32 text-center border  rounded-lg mx-1" id="days" placeholder='enter no of days' onChange={(e)=>handleChange(e)} />
            <input type="number" className="w-30 text-center border  rounded-lg mx-1" min={0} max={23} required value={todo.hours} placeholder='no of hours' id="hours"  onChange={(e)=>handleChange(e)} />
</div>


<div className='w-full  py-2 flex '> 

          <label htmlFor="status">Status:</label>  <select value={todo.status}required className='w-[50%] rounded-lg text-center mx-2' id="status" onChange={(e)=>handleChange(e)}>
